...
class MSVDVQADataset(VideoQADataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths, num_frames, prompt='', split='train'):
        ...
        for ann_path in ann_paths:
            for sample in json.load(open(ann_path)):
                self.annotation.update({sample['question_id']: sample})
    ...
    def __getitem__(self, index):
        ...
        for frame_index in selected_frame_index:
            frame = Image.open(os.path.join(self.vis_root, ann['video'].split('.')[0], "frame{:06d}.jpg".format(frame_index + 1))).convert("RGB")
